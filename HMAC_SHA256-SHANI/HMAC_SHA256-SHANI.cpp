...
std::vector<uint8_t> hmac_sha256(std::vector<uint8_t>& data, std::vector<uint8_t>& key);
...
std::vector<uint8_t> hmac_sha256(std::vector<uint8_t>& data, std::vector<uint8_t>& key) {
    ...
    static std::vector<uint8_t> paddedkey(64);// thread_local
    if (key_length > 64) {
    ...
    //(K0 ⊕ opad) || H((K0 ⊕ ipad) || text)の計算
    static std::vector<uint8_t> ipad(64 + data.size());// thread_local
    static std::vector<uint8_t> opad(96);// thread_local
    static std::vector<uint8_t> ihash(32);// thread_local

    static size_t predata_size = data.size();// thread_local
